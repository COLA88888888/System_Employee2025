<template lang="">
    ...
    <div class="modal fade" id="FormBreakType" data-bs-backdrop="static" tabindex="-1" style="display: none;" aria-hidden="true">
      <div class="modal-dialog">
          <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="backDropModalTitle">{{ $t('form_break_type') }}</h5>
                <button type="button"  class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                  <div class="row">
                      <div class="col mb-6">
                          <label for="nameBackdrop" class="form-label">{{ $t('break Type') }}: <span class="text-danger">*</span></label>
                          <input type="text" id="nameBackdrop" v-model="name" class="form-control" :placeholder="$t('enter_break_type')">
                      </div>
                    </div>
              </div>
              <div class="modal-footer">
                  <button type="button" class="btn btn-primary" :disabled="CheckForm" @click="SaveBreakType()">{{ $t('save') }}</button>
...
export default {
  ...
  methods: {
    ...
    SaveBreakType() {
        ...
        axios.post("/api/break_type/add",
          ...
          .then((response) => { 
            if (response.data.success) {
              this.name = "";
              $("#FormBreakType").modal("hide"); //ບັນທຶກຂໍ້ມູນແລ້ວຍແລ້ວປຟອມຫາຍໄປ
              this.GetBreakType(); //ບັນທືກຂໍ້ມູນສຳເລັດແລ້ວສະແດງໃນຕາຕະລາງ
        ...
        // update Department
        axios.post("/api/break_type/update/" + this.EditID,
            {
              name: this.name,
            },
    ...
    DeleteBreakType(id) {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          axios
            .delete("/api/break_type/delete/" + id, {
              headers: {
                Authorization: "Bearer " + this.staff.getToken,
              },
            })
            .then((response) => {
              if (response.data.success) {
                this.GetBreakType(); // refresh category list
                this.$swal({
                  //position: "top", // ຕຳແໜ່ງຂອງການແຈ້ງເຕືອນຢູ່ທິ່ນສຸດ
                  icon: "success", //ໄອຄອນສຳເລັດ
